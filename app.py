...
def draw_circular_horoscope(year, month, day, hour, minute):
    ...
    # Ensure matplotlib doesn't clip the outer text boundaries
    ax.set_rmax(16.5)
    ...
    for angle in range(0, 360, 5):
        ...
        if angle % 30 == 0:
            ...
            # Rashi Number (1-12) moved to innermost circle (radius 2.0)
            ax.text(np.radians(angle + 15), 2.0, str((angle // 30) + 1), ha='center', va='center', fontsize=22, fontweight='bold', color='white', zorder=2)
        else:
    ...
    # --- DYNAMIC: Rotating Market Zones ---
    for i, (start_deg, text, color) in enumerate(market_zones):
        ...
        # --- NEW: Outer House Numbers (1 to 12) added on outermost circle ---
        ax.text(theta, 15.5, str(i + 1), ha='center', va='center', fontsize=22, fontweight='bold', color='white')
